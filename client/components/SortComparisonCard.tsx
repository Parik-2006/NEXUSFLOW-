  ...
  return (
    <View style={styles.card}>
      ...
      <View style={styles.explain}>
        <Text style={styles.explainHead}>Why Merge Sort wins</Text>
        <Text style={styles.explainTxt}>
          Bubble Sort compares every adjacent pair on each pass — n×(n−1)/2 comparisons
          in the worst case, so work grows quadratically (O(n²)). Merge Sort recursively
          halves the list and merges, giving Θ(n log n) in all cases and stability that
          preserves the greedy/priority tie order.
        </Text>
        {bubble.comparisons > 0 && (
          <Text style={styles.explainStat}>
            At n = {analytics.n}, Bubble made {bubble.comparisons} comparisons vs Merge's{" "}
            {merge.comparisons} — a {(bubble.comparisons / Math.max(1, merge.comparisons)).toFixed(1)}× reduction.
          </Text>
        )}
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  explain: { marginTop: 8, padding: 12, backgroundColor: "#f9fafb", borderRadius: 10, gap: 6 },
  explainHead: { fontSize: 13, fontWeight: "700", color: "#344054" },
  explainTxt: { fontSize: 12, color: "#475467", lineHeight: 18 },
  explainStat: { fontSize: 12, color: "#4f46e5", fontWeight: "600" },
});
