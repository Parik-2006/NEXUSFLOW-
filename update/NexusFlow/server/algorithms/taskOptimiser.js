...
// ─────────────────────────────────────────────────────────────────────────────
export function greedySchedule(tasks) {
...
export function topologicalSort(tasks) {
  ...
  const inStack = new Set();   // grey nodes — cycle detection
  const order   = [];

  function dfs(id) {
    if (visited.has(id)) return true;
    if (inStack.has(id)) return false;   // back-edge → cycle

    inStack.add(id);
    const task = taskMap.get(id);
    for (const depId of (task?.dependencies ?? [])) {
      if (!dfs(String(depId))) return false;
    }
    inStack.delete(id);
    visited.add(id);
    order.push(id);   // post-order finish
    return true;
  }

  for (const task of tasks) {
    if (!dfs(sid(task))) return { sorted: null, hasCycle: true };
  }

  // Reverse post-order = topological order.
  const sorted = order.reverse().map((id) => taskMap.get(id)).filter(Boolean);
...
export function bfsDependencyLevels(tasks) {
  const sid    = (x) => String(x._id ?? x);
  const inDeg  = new Map(tasks.map((t) => [sid(t), 0]));
  const adj    = new Map(tasks.map((t) => [sid(t), []]));  // dep → [dependents]

  for (const t of tasks) {
    for (const dep of (t.dependencies ?? [])) {
      const d = String(dep);
      inDeg.set(sid(t), (inDeg.get(sid(t)) ?? 0) + 1);
      if (!adj.has(d)) adj.set(d, []);
      adj.get(d).push(sid(t));
    }
  }
  ...
  return levels;   // levels[0] can start immediately; levels[1] after that; …
}
...
export function computeRecommendation(allTasks, sprintCapacity = 20) {
  ...
  // ── Phase 2 : Greedy — rank ready tasks by value/effort ratio ─────────────
  const greedyRanked = greedySchedule(ready);
  ...
  const algorithmSummary = {
    ...
    greedy: {
      name       : "Greedy Scheduling (Value / Effort Ratio)",
