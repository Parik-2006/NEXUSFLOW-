 ...
 * greedySortTasks      : O(n log n) – dominated by Array.sort()
 *                        (V8 uses TimSort: O(n log n) worst-case).
 * Space                : O(n)   – output array, no auxiliary structures
 *                        beyond the comparator stack frame.
 *
 * SCORE FORMULA
 * -------------
 ...
 * returns a **new array** sorted in descending priority order.
 *
 ...
 * predictable for sprint planning.
 *
...
export function greedySortTasks(tasks) {
  return [...tasks].sort((a, b) => {
    ...
    // Tie-break: older task first (FIFO fairness).
