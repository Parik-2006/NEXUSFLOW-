  ...
  try {
    ...
    const plan = await fetchJson(`${SERVER_URL}/api/projects`, {
      ...
      body: JSON.stringify({
        ...
        description: "Build a student attendance tracker with QR codes, classroom analytics, and parent notifications. The system will handle 500 students across 30 classes with real-time attendance updates. Frontend built with React Native, backend with Node.js + Express, MongoDB Atlas for storage, JWT authentication, role-based access for teachers and students, push notifications via Firebase Cloud Messaging, QR code generation and scanning using device cameras, automated attendance reports in CSV/PDF, and an analytics dashboard showing attendance patterns and trends over time. The platform also provides seamless integration with school information systems, student ID card printing pipelines, daily absent SMS alerts to guardians, teacher leave substitution scheduling, and granular audit logging for administrative compliance. The system is designed for high reliability, offline caching when school Wi-Fi disconnects, end-to-end encryption for sensitive minor records, and automated nightly backups across geographically redundant cloud regions.",
        domain: "education",
        projectType: "integration_test",
      }),
    }, tokens[0]);
